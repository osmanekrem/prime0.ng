...
export const codeOptimizerAgent = inngest.createFunction(
  ...
  async ({ event, step, publish }) => {
    ...
    const sandboxTemplateId = await step.run(
      "select-sandbox-template",
      async () => {
        switch (detectedLanguage) {
          ...
          default:
            throw new Error(
              `No sandbox configured for language: ${detectedLanguage}`
            );
        }
      }
    );
    ...
    const optimizerAgent = createAgent<AgentState>({
      ...
      description:
        "An expert agent that analyzes code performance, suggests improvements, and provides benchmarks.",
    ...
    const isError =
      !finalReport || Object.keys(result.state.data.files || {}).length === 0;
    ...
    if (isError) {
      ...
      await db
        ...
        .values({
          ...
          content:
            "I'm sorry, I encountered a problem while analyzing your code. Please try again later.",
    ...
    await step.run("format-generated-code", async () => {
      await publish(
        projectChannel(event.data.projectId).status({
          status: "running",
          message: "Auto-formatting generated code...",
        })
      );

      const sandbox = await getSandbox(sandboxId);
      try {
        await sandbox.commands.run("prettier --write *");
      } catch (error: any) {
        // Prettier bile bir hata verirse, kod gerçekten çok bozuk demektir.
        console.error("Prettier failed to format the code:", error.stderr);
        throw new Error(`Auto-formatting failed: ${error.stderr}`);
      }
    });

    const benchmarkData = await step.run(
      ...
      async () => {
        ...
        const originalCode = await sandbox.files.read("original_code.js");
        const improvedCode = await sandbox.files.read("improved_code.js");
        console.log("--- EXECUTING BENCHMARK ON THE FOLLOWING FILES ---");
        console.log("--- original_code.js ---\n", originalCode);
        console.log("--- improved_code.js ---\n", improvedCode);
        console.log("-------------------------------------------------");

        let benchmarkCommand = "";
        switch (detectedLanguage) {
          case "javascript":
            benchmarkCommand = "node benchmark_runner.mjs";
            break;
          case "python":
          ...
          default:
            throw new Error(
              `No benchmark command configured for language: ${detectedLanguage}`
            );
        }

        try {
          const { stdout } = await sandbox.commands.run(benchmarkCommand);
          return JSON.parse(stdout);
        } catch (error: any) {
          if (error.name === "TimeoutError") {
            console.warn(
              "Benchmark timed out. The original algorithm is likely too inefficient."
            );
            ...
            return {
              ...
              error: "TimeoutError",
              message:
                "The execution of the original algorithm exceeded the 3-minute time limit and could not be benchmarked.",
            };
          } else {
            console.error(
              "Benchmark execution failed with a non-timeout error. Stderr:",
              error.stderr
            );
            throw new Error(
              `Benchmark execution failed: ${error.stderr || error.message}`
            );
          }
        }
      }
    ...
    const { output: finalUserResponse } =
      await responseGenerator.run(finalReport);
